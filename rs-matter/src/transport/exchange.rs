...
use super::super::wait::wait_timeout;
...
use crate::interaction_model::{self};
...
impl ExchangeId {
    ...
    async fn recv<'a>(&self, matter: &'a Matter<'a>) -> Result<RxMessage<'a>, Error> {
        ...
        loop {
            ...
            let mut timeout = pin!(wait_timeout(
                Instant::now() + Duration::from_millis(RetransEntry::max_delay_ms() * 3 / 2)
            ));
    ...
    async fn wait_tx<'a>(&self, matter: &'a Matter<'a>) -> Result<TxOutcome, Error> {
        if let Some(delay) = self.retrans_delay_ms(matter)? {
            ...
            loop {
                ...
                let mut timer = pin!(wait_timeout(expired));
    ...
    pub async fn accept_after(
        ...
        if received_timeout_ms > 0 {
            ...
            loop {
                ...
                let mut timer = pin!(wait_timeout(
                    Instant::now() + Duration::from_millis(received_timeout_ms as u64)
                ));
