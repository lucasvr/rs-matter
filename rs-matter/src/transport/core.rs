...
use crate::utils::ifmutex::{IfMutex, IfMutexGuard};
...
impl<'m> TransportMgr<'m> {
    ...
    async fn process_accept_timeout_rx(&self) -> Result<(), Error> {
        loop {
            ...
            let mut timer = pin!(wait_timeout(Instant::now() + Duration::from_millis(50)));
    ...
    async fn process_dropped_exchanges(&self) -> Result<(), Error> {
        loop {
            ...
            if wait {
                let mut timeout = pin!(wait_timeout(Instant::now() + Duration::from_millis(100)));
